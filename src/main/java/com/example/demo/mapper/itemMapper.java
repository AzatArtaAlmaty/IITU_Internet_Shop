...
public class itemMapper {
    ...
    public static ItemDto EntityInDto(ItemEntity dto, Boolean all) {
    ...
    public static List<ItemDto> ListEntityInDto(List<ItemEntity> dtos, Boolean all) {
        List<ItemDto> items = new ArrayList<>();
        for (int i = 0; i < dtos.size(); i++) {
            items.add(EntityInDto(dtos.get(i), all));
        }
        return items;
    }
}
